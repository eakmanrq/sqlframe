...
* Directly importing the `sqlframe.gizmosql` package 
...
If converting a PySpark pipeline, all `pyspark.sql` should be replaced with `sqlframe.gizmosql`.
In addition, many classes will have a `GizmoSQL` prefix. 
For example, `GizmoSQLDataFrame` instead of `DataFrame`.
...
from sqlframe.gizmosql import GizmoSQLDataFrame
```
...
from sqlframe.gizmosql import GizmoSQLSession, functions as F
...
`SparkSession` will now be a SQLFrame `GizmoSQLSession` object and everything will be run on GizmoSQL directly.
...
## Using GizmoSQL Unique Functions

GizmoSQL may have a function that isn't represented within the PySpark API. 
...
table_employee = session.table("employee")  # This object is of Type GizmoSQLTable
```
