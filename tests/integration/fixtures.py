        ...
        def compare_schemas(schema_1, schema_2):
            for schema in [schema_1, schema_2]:
                for struct_field in schema.fields:
                    # PySpark 4 changed StructField.__eq__ to use __dict__ comparison.
                    # It also populates metadata with internal keys (e.g. __autoGeneratedAlias)
                    # on aggregation columns, causing schema equality to fail even when
                    # name/type/nullable match. Clear metadata so the comparison is meaningful.
