...
def test_write_json_error(
    ...
    with pytest.raises((FileExistsError, AnalysisException)):
        df_employee.write.json(str(temp_json), mode="error")
...
def test_write_parquet_unsupported_modes(
    ...
    if isinstance(
        session,
        (SparkSession, PySparkSession),
    ):
        pytest.skip("Spark parquet writer supports 'append'")
...
def test_write_csv_append(get_session: t.Callable[[], _BaseSession], tmp_path: pathlib.Path):
    ...
    if isinstance(session, DuckDBSession):
        ...
        df_result = session.read.csv(str(temp_csv), inferSchema=True)
        assert sorted(df_result.collect()) == sorted([Row(_1=1), Row(_1=2)])
